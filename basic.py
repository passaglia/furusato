...
titles = {
    "en": r"\textbf{Furusato n\={o}zei is exploding in popularity}",
    "jp": r"\textbf{ふるさと納税の急速な拡大}",
}
scalings = {"en": 10**9, "jp": 10**8}
subtitles = {"en": r"Total donations by year", "jp": r"合計寄附金額"}
xlabels = {"en": "", "jp": r""}

langs = ["en", "jp"]
predatayears = list(range(2008, 2016))
predatadonations = list(
    np.array([81.4, 77.0, 102.2, 121.6, 104.1, 145.6, 388.5, 1652.9]) * 10**8
)  # yen
...
for lang in langs:
    ...
    ax.set_xlim(min(annual_df.year) - 0.5, max(annual_df.year) + 0.5)
    ax.set_xticks([2009, 2011, 2013, 2015, 2017, 2019, 2021])
    ax.plot(
        predatayears + list(annual_df.year),
    ...
    for suffix in output_filetypes:
        fig.savefig(
            PLOT_FOLDER + suffix + "/" + "total_donations_" + lang + "." + suffix,
            transparent=True,
            bbox_inches="tight",
        )
    plt.close("all")
...
titles = {
    "en": r"\textbf{Half of the money is lost to expenses}",
...
colorstring = (
    ...
    + "}"
    + r"\definecolor{green}{rgb}{"
    + str(samcolors.nice_colors(4))[1:-1]
    + "}"
...
subtitles = {
    "en": colorstring
    + r"Fraction of donation money going to the \textbf{\textcolor{blue}{cost of gifts}},"
    + "\n"
    + colorstring
    + r"\textbf{\textcolor{green}{shipping}}, and \textbf{\textcolor{red}{administrative costs, marketing, and fees}}",
    "jp": colorstring
    + r"寄附金額のうち、\textbf{\textcolor{blue}{返礼品の調達}}、\textbf{\textcolor{green}{送付}}、"
    + "\n"
    + colorstring
    + r"\textbf{\textcolor{red}{事務、 広報、 決済等}}、にかかる費用の割合",
}
...
for lang in langs:
    ...
    ax.set_xlim(min(annual_df.year) - 0.4, max(annual_df.year))
    ax.plot(
        list(annual_df.year),
        list(100 * annual_df["product-cost"] / annual_df["donations"]),
        color=samcolors.nice_colors(3),
    )
    ax.fill_between(
        list(annual_df.year),
    ...
    ax.plot(
        ...
        list(
            100
            * (annual_df["product-cost"] + annual_df["shipping-cost"])
            / annual_df["donations"]
        ),
        color=samcolors.nice_colors(4),
    )
    ax.fill_between(
        list(annual_df.year),
        list(100 * annual_df["product-cost"] / annual_df["donations"]),
    ...
    ax.plot(
        list(annual_df.year),
        list(100 * annual_df["total-cost"] / annual_df["donations"]),
...
ax.set_title(subtitle_string, x=0.0, y=1.05, fontsize=14, ha="left", va="bottom")
fig.suptitle(
    title_string,
    x=0,
    y=1.2,
...
colors = [
    samcolors.nice_colors(i / len(annual_df.year)) for i in range(len(annual_df.year))
]
for i, year in enumerate(annual_df.year):
    ...
    print(len(vals))
    print(
        np.round(100 * (1 - len(vals[np.where(vals > 0)]) / len(vals))),
        "% of places lose money",
    )
...
for suffix in output_filetypes:
    fig.savefig(
        PLOT_FOLDER + "/" + suffix + "/" + "percentile." + suffix, transparent=True
    )
plt.close("all")
...
for suffix in output_filetypes:
    fig.savefig(
        PLOT_FOLDER + "/" + suffix + "/" + "histogram." + suffix, transparent=True
    )
plt.close("all")
...
for i, year in enumerate(annual_df.year):
    ...
    ax.plot(
        np.arange(1, len(donations) + 1) / len(donations) * 100,
        donations.cumsum() / donations.sum(),
        color=colors[i],
        label=year,
    )
ax.legend()
...
for suffix in output_filetypes:
    fig.savefig(
        PLOT_FOLDER + "/" + suffix + "/" + "cumulative_donations." + suffix,
        transparent=True,
    )
plt.close("all")
...
langs = ["en", "jp"]
titles = {
    "en": r"\textbf{Winners take a huge chunk of the money}",
    "jp": r"\textbf{トップ自治体は寄附金の莫大なシェアを占める}",
}
subtitles = {
    "en": r"Proportion of donations going to \textbf{top "
    + str(int(topN))
    + "} local governments (of "
    + str(
        np.sum(
            ~np.isnan(furusato_df.loc[furusato_df["year"] == 2020]["donations"].values)
        )
    )
    + ")",
    "jp": r"受入額が多い\textbf{"
...
for lang in langs:
    ...
    for suffix in output_filetypes:
        fig.savefig(
            PLOT_FOLDER + "/" + suffix + "/" + "donation_topN_" + lang + "." + suffix,
            transparent=True,
            bbox_inches="tight",
        )
    plt.close("all")
    shutil.copy(
        PLOT_FOLDER + "pdf/donation_topN_" + lang + ".pdf", productionPlotFolder
    )
...
topN = 10
titles = {
    "en": r"\textbf{Municipalities which receive the most donations}",
    "jp": r"\textbf{ふるさと納税受入額の多い10自治体}",
}
...
for lang in langs:
    ...
    fig.suptitle(titles[lang], y=0.92, fontsize=cbtable.titlesize)
    code_list = []
    names_list = []
    for i, year in enumerate(annual_df.year):
        inds = np.argsort(
            furusato_df.loc[furusato_df["year"] == year]["donations"].values
        )[::-1]
        code_list.append(
            furusato_df.loc[furusato_df["year"] == year]["code"]
            .iloc[inds][0:topN]
            .values
        )
        names_list.append(
            [
                s.title().replace("ou", r"\={o}")
                for s in furusato_df.loc[furusato_df["year"] == year][colname]
                .iloc[inds][0:topN]
                .values
            ]
        )
    ...
    for suffix in output_filetypes:
        fig.savefig(
            PLOT_FOLDER + "/" + suffix + "/" + "topN_table_" + lang + "." + suffix,
            transparent=True,
            bbox_inches="tight",
        )
    plt.close("all")
...
for lang in langs:
    ...
    for i, year in enumerate(annual_df.year):
        ...
        code_list.append(
            local_df.loc[local_df["year"] == year]["code"].iloc[inds][0:topN].values
        )
        names_list.append(
            [
                s.title().replace("ou", r"\={o}")
                for s in local_df.loc[local_df["year"] == year][colname]
                .iloc[inds][0:topN]
                .values
            ]
        )
    ...
    shutil.copy(
        PLOT_FOLDER + "pdf/topN_profit_table_" + lang + ".pdf", productionPlotFolder
    )
