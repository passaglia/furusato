...
pref_map_df = pref_map_df_loader(quality='medium')
local_map_df = local_map_df_loader(quality='medium')
...
def make_pref(PREFECTURE):
    ...
    df = local_map_df.loc[local_map_df['year']==year].copy()
    ...
    pref_df=pref_map_df.loc[(pref_map_df['prefecture']==PREFECTURE) & (pref_map_df['year']==year)].copy()
    ...
    PREFECTURE_EN = (pref_df['prefecture-reading'].iloc[0]).replace('ou','o').replace('oo','o').title() 
    ...
    for lang in langs:
        ...
        if PREFECTURE != '北海道':
            pref_df.plot(ax=ax,edgecolor=(0,0,0),facecolor="None",lw=.3)
        ...
        boundsarr = pref_df.to_crs(epsg=3857).bounds.values[0]
        center =  pref_df.to_crs(epsg=3857).centroid.iloc[0].coords.xy
