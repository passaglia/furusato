...
fig, ax = cb.handlers()
title = r"Furusato nouzei: A program on the rise"
subtitle = r"Received, Reported, Deducted, Products (¥100m)"
cb.set_titleSubtitle(ax, title, subtitle)
...
for lang in langs: 
    ...
    ax.plot(predatayears+list(furusato_arr.year.values), np.array(predatadonations+list(furusato_arr['donations'].sum(dim='prefecturecity')))/scalings[lang], color=samcolors.nice_colors(3))
...
for lang in langs: 
    ...
    ax.set_xlim(min(furusato_arr.year)-.4,max(furusato_arr.year))
...
subtitles = {'en':r"Proportion of donations going to \textbf{top " + str(int(topN)) + "} local governments (of "  + str(np.sum(~np.isnan(furusato_arr.loc[{'year':2020}]['donations'].values))) + ')','jp':r"受入額が多い\textbf{"+str(int(topN))+r"}自治体が占める全寄附金の割合（合計" + str(np.sum(~np.isnan(furusato_arr.loc[{'year':2020}]['donations'].values))) + 'ヶ所）'}
...
for lang in langs:
    ...
    # cb.set_byline(ax, "Sam Passaglia")
    ax.set_ylim(0,51)
    ax.set_xlim(min(furusato_rough_df.year)-1.7,max(furusato_rough_df.year)+.75)
...
langs = ['en','jp']
for lang in langs:
    cbtable = CircusBoy(baseFont=['Helvetica','Hiragino Maru Gothic Pro'],titleFont=['Helvetica','Hiragino Maru Gothic Pro'],textFont=['Helvetica','Hiragino Maru Gothic Pro'], fontsize=12,figsize=(6.75,4))
