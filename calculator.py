...
CALCULATOR_FILE = "./calculator/calculator_edit.xlsx"
...
def maxDonationBreakdown(income):
    sheet[incomeCell].value = income
    return (
        sheet[maxIncomeTaxDeductionCell].value,
...
save_folder = "./calculator/"

man = 10000
incomes = np.linspace(0.1 * man, 3000 * man, 200)
...
titles = {
    "en": r"\textbf{High earners can get many more gifts}",
    "jp": r"\textbf{ふるさと納税の逆進性}",
}
subtitles = {
    "en": r"Proportion of income which is eligible for donation",
    "jp": r"給与収入による最大寄付額の割合",
}
xlabels = {"en": "Personal income (￥mn)", "jp": r"給与収入 （万円）"}
...
for lang in langs:
    ...
    ax.plot(
        incomes / scalings[lang],
        np.array(deductions / incomes) * 100,
        color=nice_colors(3),
    )
    ...
    # ax.axis('tight')
    fig.savefig(
        save_folder + "fracdeduction_vs_income_" + lang + ".pdf",
        bbox_inches="tight",
        transparent=True,
    )
...
shutil.copy(save_folder + "fracdeduction_vs_income_en.pdf", productionPlotFolder)
shutil.copy(save_folder + "fracdeduction_vs_income_jp.pdf", productionPlotFolder)
...
scalings = {"en": 10**6, "jp": 10000}
titles = {
    "en": r"\textbf{Ratio of 住民税控除　to 所得税控除}",
    "jp": r"\textbf{給与収入により、住民税控除/所得税控除の率}",
}
subtitles = {
    "en": r"If you donated the max amount ",
...
for lang in langs:
    ...
    fig, ax = cb.handlers()
    ax.set_title(subtitles[lang], x=0.0, y=1.01, fontsize=14, ha="left", va="bottom")
    fig.suptitle(
        titles[lang],
        x=0,
        y=1.15,
    ...
    ax.plot(
        incomes / scalings[lang],
        np.array((breakdowns[:, 1] + breakdowns[:, 2]) / breakdowns[:, 0]),
        color=nice_colors(1),
    )
    ...
    # ax.axis('tight')
    fig.savefig(
        save_folder + "ratio_vs_income_" + lang + ".pdf",
        bbox_inches="tight",
        transparent=True,
    )
...
for lang in langs:
    ...
    ax.plot(
        incomes / scalings[lang],
        np.array((breakdowns[:, 1] + breakdowns[:, 2])),
        color=nice_colors(1),
    )
    ...
    # ax.axis('tight')
    fig.savefig(
        save_folder + "deduction_vs_income_" + lang + ".pdf",
        bbox_inches="tight",
        transparent=True,
    )
